...
y = x.pow(2) + 0.1*torch.randn(x.size())                 # noisy y data (tensor), shape=(100, 1)
...
for epoche in range(150):
    ...
    plt.subplot(211)
    plt.plot(x.data.numpy(), y.data.numpy(), '.', ms=5)
    plt.plot(x.data.numpy(), prediction.data.numpy(), 'r-', lw=1)
    plt.text(0.5, -0.2, 'Loss=%.6f' % loss.data.numpy(), fontdict={'size': 12, 'color':  'red'})
