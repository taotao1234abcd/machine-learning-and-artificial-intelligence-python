...
class PSO:
    def __init__(self,
                 ...
                 w_start=0.9,
                 w_end=0.4,
                 w_method=pso_w_method.linear_decrease,
        ...
        self.targets_func = np.zeros(population_size)
    ...
    def run(self):
        ...
        print("-" * 20, "generation %2d" % 0, "-" * 20)
        for k in range(self.population_size):
            # print('%2d' % k, end='  ')
            self.targets_func[k] = self.func(self.population[k])

        self.best_individuals_points = deepcopy(self.population)
        self.best_individuals_targets = deepcopy(self.targets_func)
        if self.func_type == pso_config.func_type_min:
            self.best_population_target = np.min(self.targets_func)
            best_population_index = np.argmin(self.targets_func)
            self.best_population_point = self.population[best_population_index].copy()
        else:
            self.best_population_target = np.max(self.targets_func)
            best_population_index = np.argmax(self.targets_func)
            self.best_population_point = self.population[best_population_index].copy()
        ...
        for i in range(self.generations):
            ...
            elif self.w_method == pso_w_method.exp:
            ...
            if self.complex_constraints is not None:
                for k in range(self.population_size):
                    if self._check_constraints(self.population[k]) > 0:
                        self.population[k] = self._loop_v_modify(self.population[k], k, w_)

            print("-" * 20, "generation %2d" % (i + 1), "-" * 20)
            for k in range(self.population_size):
                # print('%2d' % k, end='  ')
                self.targets_func[k] = self.func(self.population[k])
            ...
            for k in range(self.population_size):
                ...
                if (self.targets_func[k] < self.best_individuals_targets[k] and self.func_type == pso_config.func_type_min) or \
                   (self.targets_func[k] > self.best_individuals_targets[k] and self.func_type == pso_config.func_type_max):
                    self.best_individuals_points[k] = self.population[k].copy()
                    self.best_individuals_targets[k] = self.targets_func[k].copy()
                if (self.targets_func[k] < self.best_population_target and self.func_type == pso_config.func_type_min) or \
                   (self.targets_func[k] > self.best_population_target and self.func_type == pso_config.func_type_max):
                    self.best_population_point = self.population[k].copy()
                    self.best_population_target = self.targets_func[k].copy()
    ...
    def show_result(self):
        ...
        #         print("%s:%s" %(k,v))
        print("-" * 20, "PSO calculation result", "-" * 20)
        # print("global best generation index/total generations: %s/%s" %(self.global_best_index,self.generations))
        print("global best point:", self.generations_best_points[self.global_best_index])
        print("global best target:", self.generations_best_targets[self.global_best_index])
