...
for index_train, index_test in kf.split(data_scaled):
    ...
    print('  ')
    print('交叉验证:', kf_times, '/', n_splits)
    print('平均训练误差:', train_mean_error)
    train_mean_error_list.append(train_mean_error)
    ...
    test_mean_error_list.append(test_mean_error)



for i in range(2):
    test_mean_error_list_index = test_mean_error_list.index(max(test_mean_error_list))
    del train_mean_error_list[test_mean_error_list_index]
    del test_mean_error_list[test_mean_error_list_index]
print('  ')
print('  ')
train_mean_error = np.mean(train_mean_error_list)*100
print('平均训练误差:%8.4f' % train_mean_error, '%')
test_mean_error = np.mean(test_mean_error_list)*100
print('平均预测误差:%8.4f' % test_mean_error, '%')
