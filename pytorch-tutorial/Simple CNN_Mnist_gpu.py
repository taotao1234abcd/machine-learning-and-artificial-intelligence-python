...
train_data_y = torch.from_numpy(train_data_y).to(dtype=torch.int64).cuda()
train_data_x = torch.from_numpy(train_data_x)
train_data_x = torch.unsqueeze(train_data_x, dim=1).type(torch.FloatTensor).cuda()
...
test_data_y = torch.from_numpy(test_data_y).to(dtype=torch.int64).cuda()
test_data_x = torch.from_numpy(test_data_x)
test_data_x = torch.unsqueeze(test_data_x, dim=1).type(torch.FloatTensor).cuda()
...
labels = test_data_y.cpu().numpy()[:plot_only]
